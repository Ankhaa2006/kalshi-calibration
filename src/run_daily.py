...
def update_pnl():
    ...
    for trade in trades:
        ...
        try:
            ...
            if m.get('status') in ('settled', 'finalized'):
                ...
                if won:
                    if side == 'yes':
                        gross_profit = count * (1 - price_cents/100)
                    else:
                        gross_profit = count * (price_cents/100)
                    pnl = gross_profit * 0.93 - cost  # after 7% fee
                else:
                    pnl = -cost
    ...
    return [t for t in summary if not t.get('dry_run') == 'True']
